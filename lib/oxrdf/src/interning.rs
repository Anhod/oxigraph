...
// struct lasso::Rodeo：一个字符串中间体，以最小的内存占用快速缓存字符串，返回一个唯一的键，以O(1)时间复杂度访问它
//                      默认使用Spur作为它的key，RandomState作为hasher

// Spur：key:NonZeroU32，作为Rodeo默认的key
...
// 实现了 Hash trait可以对每个字段进行散列，通过配合Hasher使用
// 可以键 dataset.rs line 636
...
impl InternedNamedNode {
    // 根据NamedNodeRef中的iri字符串获得key，其类型为Spur，也就是InternedNamedNode中的id
    // 如果该字符串没被interning过，则将它interning；如果interning过，则什么也不做
    ...
    // 根据字符串得到 key value（应用在该字符串被interning过）
...
impl InternedLiteral {
    ...
    // 有可能LiteralRef对象没有被interned过，所以返回Option，其有可能返回的是空的
