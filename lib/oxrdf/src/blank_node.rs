...
impl BlankNode {
    ...
    pub fn new(id: impl Into<String>) -> Result<Self, BlankNodeIdParseError> {   // 验证了id再进行创建
        let id = id.into();
    ...
    // 没有检查 id 的形式
    #[inline]
    pub fn new_unchecked(id: impl Into<String>) -> Self {   // 不验证id是否合法，直接创建
        let id = id.into();

        if let Some(numerical_id) = to_integer_id(&id) {  // to_integer_id: 将字符串转化为16进制的数字（1-1,a-10,b-11)
    ...
    /// In most cases, it is much more convenient to create a blank node using [`BlankNode::default()`].
    #[inline]
    pub fn new_from_unique_id(id: u128) -> Self {  // 如果可以转化成u128则这样创建
    ...
    pub fn as_str(&self) -> &str {
        match &self.0 {
            ...
            BlankNodeContent::Anonymous { str, .. } => str.as_str(),   // 看来名称是对应的
        }
    }
    ...
    pub fn into_string(self) -> String {
        match self.0 {
            BlankNodeContent::Named(id) => id,
            BlankNodeContent::Anonymous { str, .. } => str.as_str().to_owned(),   // 有所有权的
        }
    }
...
enum BlankNodeRefContent<'a> {
    Named(&'a str),
    Anonymous { id: u128, str: &'a str },   // 匿名的
}
...
// 就是 u128整型形式 和 十六进制（内存数组）的转换
impl IdStr {
    #[inline]
    fn new(id: u128) -> Self {
        let mut str = [0; 32];    // 字符数组
        write!(&mut str[..], "{:x}", id).unwrap();  
        Self(str)
    }

    #[inline]
    fn as_str(&self) -> &str {   // 返回的是16进制（字节数组）
...
// str 转 u128
// 123a = 1 × 16的3次方 + 2 × 16的2次方 + 3 × 16的1次方 + 10
#[inline]
fn to_integer_id(id: &str) -> Option<u128> {
    ...
    for digit in digits {
        value = value.checked_mul(16)?.checked_add(
            match *digit {
                b'0'..=b'9' => digit - b'0',
                b'a'..=b'f' => digit - b'a' + 10,  // a = 10, b = 11, c = 12
                _ => return None,
            }
            .into(),
        )?;
