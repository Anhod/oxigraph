...
// 由一系列类型获得 EncodedTerm（Triple、NamedNode...）
...
// 利用 EncodedTerm 组成 EncodedTriple 和 EncodedQuad
...
pub fn insert_term<F: FnMut(&StrHash, &str) -> Result<(), StorageError>>(
    ...
    match term {
        ...
        TermRef::Triple(triple) => {   // 如果是三元组的话，把主谓宾再调用该方法一遍
...
pub trait Decoder: StrLookup {
    // 先用该方法确定EncodedTerm的类型（Term），再使用下面的方法构造目标Term
    ...
    fn decode_subject(&self, encoded: &EncodedTerm) -> Result<Subject, StorageError> {
        match self.decode_term(encoded)? {
            Term::NamedNode(named_node) => Ok(named_node.into()),  // 用 NamedNode 构造 Subject（其中实现了 From trait）
