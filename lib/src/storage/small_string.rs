...
impl SmallString {
    ...
    // from_str 方法将字节切片序列转化为&str（需要控制一下长度）
    #[inline]
    ...
    pub fn from_be_bytes(bytes: [u8; 16]) -> Result<Self, BadSmallStringError> {
        ...
        str::from_utf8(&bytes.as_ref()[..bytes[15].into()]) // 啊这个调用只是为了来检验bytes是不是合格的
...
impl FromStr for SmallString {
    ...
    fn from_str(value: &str) -> Result<Self, BadSmallStringError> {
        if value.len() <= 15 {
            let mut inner = [0; 16];
            inner[..value.len()].copy_from_slice(value.as_bytes());  // copy_from_slice: 使用 memcpy 将所有元素从 src 复制到 self。
            inner[15] = value.len().try_into()
