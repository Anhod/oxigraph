...
// 数据库的实现，提供了get(key)、snapshot、contains_ket(key)等操作
impl Db {
    // new()和open()的区别是，前者使用临时路径创建数据库，后者使用给定的路径打开数据库
    // 给定了路径之后就达到了持久层的地步
    pub fn new(column_families: Vec<ColumnFamilyDefinition>) -> Result<Self, StorageError> {
        // cfg!宏：仅在目标系统是linux时才执行下面的代码
    ...
    fn do_open(
        ...
        unsafe {
            ...
            if !column_families.iter().any(|c| c.name == "default") {   // 测试迭代器的元素是否与谓词匹配
    ...
    pub fn transaction<'a, 'b: 'a, T, E: Error + 'static + From<StorageError>>(
        ...
        loop {
            let transaction = unsafe {
                let transaction = rocksdb_transaction_begin(    // 开启事务
                    self.0.db,
            ...
            let result = f(Transaction {   // 构建的事务结构体，并将闭包结果返回调用的地方
    ...
    // insert 函数，但是键和值是u8类型（1bytes）
    // 将（key,value）绑定到对应的 cf 上
    pub fn insert(
        &self,
        column_family: &ColumnFamily,
        key: &[u8],
    ...
    // 当 Level 0 层的 SST 文件个数超过阈值之后，就会通过 Compaction 策略将其放到 Level 1 层，以此类推，直到最底层
...
impl Reader {
    // 对 InnerReader 分开讨论，分为 Snapshot 和 Transaction
    pub fn get(
        &self,
        column_family: &ColumnFamily,
        key: &[u8],
        ...
        unsafe {
            let slice = match &self.inner {
                ...
                InnerReader::Transaction(inner) => {
                    if let Some(inner) = inner.upgrade() {   // upgrade将Weak指针升级到Rc，如果成功，则延迟丢弃内部值；如果内部值已经被丢弃，则返回 None
    ...
    // 给定 prefix 查询满足的三元组（prefix是字节数组，三元组模式之一）
    pub fn scan_prefix(
        &self,
        column_family: &ColumnFamily,
        prefix: &[u8],
        ...
        let upper_bound = {
            ...
            let mut found = false;
            for c in bound.iter_mut().rev() {  // 获得反转方向的（从右到左）的可修改值的迭代器
                if *c < u8::MAX {
                    *c += 1;
                    found = true;
                    break;
    ...
    pub fn insert(
        ...
        unsafe {
            ffi_result!(rocksdb_transaction_put_cf_with_status(
                *self.transaction,
                column_family.0,
                key.as_ptr() as *const c_char,  // as_ptr获取到的指针是堆中字节序列的指针地址,而&a的地址是字符串变量在栈上的指针地址
                key.len(),
                value.as_ptr() as *const c_char,
                value.len(),
            ))?;
...
impl Deref for PinnableSlice {
    ...
    fn deref(&self) -> &[u8] {
        unsafe {
            ...
            slice::from_raw_parts(val as *const u8, len)   // 根据指针和长度形成切片
        }
    }
}
...
impl SstFileWriter {
    ...
    // TODO：该方法留着将区间编码全加在key里
    pub fn insert_empty(&mut self, key: &[u8]) -> Result<(), StorageError> {
        self.insert(key, &[])
    }

    // TODO：使用该方法进行key、value的插入
    pub fn insert_key_value(&mut self, key: &[u8], value: &[u8]) -> Result<(), StorageError> {
