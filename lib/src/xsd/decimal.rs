...
impl Decimal {
    ...
    pub fn checked_add(&self, rhs: impl Into<Self>) -> Option<Self> {
        Some(Self {
            value: self.value.checked_add(rhs.into().value)?,  // 相加操作，检查结果是否溢出，溢出则返回 Node
        })
    }
