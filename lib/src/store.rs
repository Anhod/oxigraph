...
impl Store {
    ...
    // 调用到mod.rs中StorageReader中的代码生成给定模式的元组迭代器
    pub fn quads_for_pattern(
        ...
        let reader = self.storage.snapshot();   // 得到快照
    ...
    // 和下面的 load_datasets 有什么区别（从参数来看就少了个图）
    pub fn load_graph<'a>(
        &self,
        reader: impl BufRead,
        format: GraphFormat,
...
impl BulkLoader {
    ...
    // 1、将区间编码加入value中
    pub fn load_graph_oxiuse_value<'a>(
        &self,
        reader: impl BufRead,
        format: GraphFormat,
        to_graph_name: impl Into<GraphNameRef<'a>>,
        ...
        self.storage
            .load_oxiuse_value(parser.read_triples(reader)?.filter_map(|r| match r {   // TODO：更换方法
                Ok(q) => Some(Ok(q.in_graph(to_graph_name.into_owned()))),
    ...
    // 2、将区间编码加入key中
    pub fn load_graph_oxiuse_key<'a>(
        &self,
        reader: impl BufRead,
        format: GraphFormat,
        to_graph_name: impl Into<GraphNameRef<'a>>,
        ...
        // let trees = self.construct_tree(path).unwrap();   // 传入数据集获得 tree
        ...
        self.storage
            .load_oxiuse_key(parser.read_triples(reader)?.filter_map(|r| match r {   // TODO：更换方法
                Ok(q) => Some(Ok(q.in_graph(to_graph_name.into_owned()))),
